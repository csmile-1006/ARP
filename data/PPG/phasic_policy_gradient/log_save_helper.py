...
def rcm(start, stop, modulus, mode="[)"):
    ...
    return (start < stop) and ((left_hit and mode[0] == "[") or (middle_hit) or (right_hit and mode[1] == "]"))
...
class LogSaveHelper:
    def __init__(
        ...
        save_period: "(int) period of saving model" = 5,
    ...
    def __call__(self):
        ...
        will_save = (self.ic_per_save > 0) and rcm(self.last_ic + 1, self.total_interact_count + 1, self.ic_per_save)
        self.log()
        if will_save:
            self.save()
        return True
    ...
    def save(self):
        ...
        elif self.save_mode == "periodic":
            if self.save_idx and self.save_idx % self.save_period == 0:
                basename = f"model{self.save_idx:03d}"
            else:
                self.save_idx += 1
                return
        ...
        basename += f"{suffix}_IC{self.total_interact_count}.jd"
