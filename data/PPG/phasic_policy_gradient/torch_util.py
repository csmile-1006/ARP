...
def batch_len(batch):
    ...
    assert all(arr.shape[0] == b for arr in flatlist if th.is_tensor(arr)), "Not all arrays have same batchsize!"
    return b
...
def torch_init_process_group(backend, comm=None, start_port=29500, attempts=10, jitter_seconds_per_rank=0.005):
    """
...
def minibatched_call(fn, mbsize, *args, **kwargs):
    ...
    mbs = [fn(*tree_slice(args, inds), **tree_slice(kwargs, inds)) for inds in th.arange(batchsize).split(mbsize)]
