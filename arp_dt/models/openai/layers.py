...
class Transformer(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, attn_mask: Optional[jnp.ndarray] = None) -> jnp.ndarray:
        for i in range(self.num_layers):
            x = ResidualAttentionBlock(num_heads=self.num_heads, name=f"resblocks.{i}")(x, attn_mask)
...
class TextEncoder(nn.Module):
    ...
    def __call__(self, text: jnp.ndarray) -> jnp.ndarray:
        ...
        x = x + positional_embedding[None]
        x = Transformer(self.features, self.num_layers, self.num_heads, name="transformer")(x, attn_mask=mask)
