...
class ResNetBlock(nn.Module):
    ...
    def __call__(self, x):
        ...
        if residual.shape != x.shape:
            residual = Conv1x1(self.filters, strides=self.strides, name="proj_conv")(residual)
...
class BottleneckResNetBlock(ResNetBlock):
    ...
    def __call__(self, x):
        ...
        if residual.shape != x.shape:
            residual = Conv1x1(4 * self.filters, strides=self.strides, name="proj_conv")(residual)
            residual = self.norm(name="proj_bn")(residual)
...
class ResNet(nn.Module):
    ...
    def __call__(self, x, *, train):
        ...
        norm = functools.partial(nn.BatchNorm, use_running_average=not train, momentum=0.9)

        # Root block
        x = nn.Conv(features=width, kernel_size=(7, 7), strides=(2, 2), use_bias=False, name="init_conv")(x)
...
ResNet50 = functools.partial(ResNet, stage_sizes=(3, 4, 6, 3), block_cls=BottleneckResNetBlock)
ResNet101 = functools.partial(ResNet, stage_sizes=(3, 4, 23, 3), block_cls=BottleneckResNetBlock)
ResNet152 = functools.partial(ResNet, stage_sizes=(3, 8, 36, 3), block_cls=BottleneckResNetBlock)
ResNet200 = functools.partial(ResNet, stage_sizes=(3, 24, 36, 3), block_cls=BottleneckResNetBlock)
